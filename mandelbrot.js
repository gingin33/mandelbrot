...
//倍率と移動座標を指定してマンデルブロ集合生成
function exeMandelbrot(mag, mx, my) {
    ...
    mandelbrot(zosh, canvas.height, moveX, moveY, n, r, g, b, m);     //上記の条件でマンデルブロ集合を生成
...
//マンデルブロ集合生成のそのものの処理
function mandelbrot(size, pix, movex, movey, n, red, green, blue, m) {
    var x;
    var y;
    var a;
    var b;
    ...
    for (var i = 0; i < pix; i++) {
        a = (i * size - (-1) * movex) / pix - size / 2;                 //実部を順番に指定
        for (var j = 0; j < pix; j++) {
            b = (j * size - (-1) * movey) / pix - size / 2;                //虚部を順番に指定
            x = 0;                                                        //マンデルブロ集合の基本として複素数の初期値は0
            y = 0;
...
function saveImage() {
    mandelbrot()
    var name = window.prompt("ファイル名を入力してください", "mandelbrotImage.png");
    if (name != null) {
        if (canvas2.msToBlob) {
